...
Adds a gene to the end of the nucleosome. If gene is None, adds a random Gene using
...
Recombines with the other Nucleosome, swapping at the given indices. If indices is
...
Returns an exact copy of the Chromosome, copying the underlying Nucleosomes as well.
...
Inserts the nucleosome at the index. If index is None, the nucleosome is inserted at a
random index. If nucleosome is None, adds a random nucleosome using Nucleosome.make, passing
...
Adds a nucleosome to the end of the chromosome. If nucleosome is None, adds a random
Nucleosome using Nucleosome.make and passing kwargs. Returns self for chaining
operations.

##### `duplicate(index: int = None) -> Chromosome:`

Duplicates the Nucleosome at the index. If index is None, the Nucleosome at a random
...
Substitutes the Nucleosome at the index with the given nucleosome. If index is None, a
random index will be used. If nucleosome is None, adds a random nucleosome using
Nucleosome.make, passing the kwargs. Returns self for chaining operations.
...
chosen. Recombines individual Nucleosomes if recombine_allels is True. Recombines
only Nucleosomes with matching names if match_nucleosomes is True. Recombines individual
...
Recombines individual Nucleosomes if recombine_nucleosomes is True. Recombines only
Nucleosomes with matching names if match_nucleosomes is True. Recombines individual
Genes if recombine_genes is True. Recombines only Genes with matching names if
